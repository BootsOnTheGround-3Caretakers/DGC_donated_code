...
function emailOrgKeysToUser(firebase_email,firebase_token, p2s9t1_user_uid, p2s9t1_org_uid) {
    return new Promise(function (resolve, reject) {
        var return_msg = "";
        var debug_data = [];
        var call_result = {};
        var task_id = "p2s9t1emailOrgKeysToUser";
        var response_data = {};

        ///// input validation
        call_result = bi1_data_validation.is_email_address(firebase_email);
        debug_data.push(call_result);
        call_result = bi1_data_validation.is_org_uid(p2s9t1_org_uid);
        debug_data.push(call_result);
        call_result = bi1_data_validation.is_user_uid(p2s9t1_user_uid);
        debug_data.push(call_result);
        ...
        ajax({
            url: window.G_ajax_test_domain + '/web-commands/p2s9t1-email-org-keys-to-user',
            method: 'POST',
            data: {
              'p2s9_firebase_email': firebase_email,
              'p2s9t1_user_uid': p2s9t1_user_uid,
              'p2s9t1_org_uid': p2s9t1_org_uid,
            ...
            error: function (result) {
              ...
              return_msg += "failed to send org keys, error data:" + JSON.stringify(result);
...
export default emailOrgKeysToUser